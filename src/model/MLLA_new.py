...
# --- 在 src/model/MLLA_new.py 中，用下面这个类完整替换你现有的 channel_MLLA 类 ---

# --- 在 MLLA_new.py 中，替换整个 channel_MLLA 类定义为下方版本 ---
...
class channel_MLLA(nn.Module):
    """
    将 to_patch 内联为类方法，避免 NameError；
    并且确保使用输入张量 device，避免硬编码 .to('cuda')
    """
    def __init__(self, context_window, patch_size, hidden_dim, out_dim, depth, patch_stride, n_heads):
        ...
        # 由上下文长度与 patch 超参计算 patch 数
        patch_num = int((context_window - patch_size) / patch_stride + 1)

        # 用你原有的 MLLA_BasicLayer（文件中已定义，无需修改）
    ...
    @staticmethod
    def to_patch(data: torch.Tensor, patch_size: int, stride: int) -> torch.Tensor:
        """
        data: [B*, T, D1]   (这里 B* = B * n_channels)
        返回: [B*, N, patch_size]
        注意：使用 data 同设备。
        """
        batchsize, timelen, dim = data.shape
        assert dim >= 1, f"to_patch 期望最后一维 >=1，当前 dim={dim}"
        num_patches = (timelen - patch_size) // stride + 1
        device = data.device
        patches = torch.zeros((batchsize, num_patches, patch_size), device=device, dtype=data.dtype)
        # 下面与原逻辑保持一致：只取 data[..., 0] 作为切片源
        # 如果你希望用所有维度，可自行改为 patches[:, i, :] = data[:, start:end, :].mean(-1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 期望 x 形状: [B, D1, n_channels, T]
        B, D1, n_channels, T = x.shape

        # 合并 batch 与 channel，得到 [B*n_channels, D1, T]
        x = x.permute(0, 2, 1, 3).reshape(B * n_channels, D1, T)

        # 变为 [B*n_channels, T, D1] 以便做时间维切 patch
        x = x.permute(0, 2, 1)

        # 切 patch -> [B*n_channels, N, patch_size]
        x = self.to_patch(x, patch_size=self.patch_size, stride=self.patch_stride)

        # 送入编码器 -> [B*n_channels, N, out_dim]
        x = self.encoder(x)

        # 还原 batch 与通道维： [B, n_channels, N, out_dim]
        x = x.view(B, n_channels, x.shape[1], x.shape[2])

        # 调整为 [B, n_channels, out_dim, N] 做分层归一化
        x = x.permute(0, 1, 3, 2)
        x = stratified_layerNorm(x, n_samples=B // 2)

        # 再切回 [B, n_channels, N, out_dim]
        x = x.permute(0, 1, 3, 2)
        return x
