...
# --- src/utils.py 中，替换原有 stratified_layerNorm 实现 ---
import math
import torch

def stratified_layerNorm(out: torch.Tensor, n_samples: int):
    """
    对 out 做“按组（每组 n_samples）标准化”。当 n_samples<=1 或 batch 太小，
    自动降级为 per-sample 标准化，避免除零或越界。
    期望输入形状: [B, D, C, T]（或 [B, C, D, N] 这类，统一把中间维度展平做列标准化）
    """
    B = out.shape[0]

    # 兜底：非法或过小的 n_samples 直接 per-sample 标准化
    if n_samples is None or int(n_samples) < 1:
        x = out.reshape(B, -1, out.shape[-1]).permute(0, 2, 1)              # [B, T, F]
        x = x.reshape(B * out.shape[-1], -1)                                 # [B*T, F]
        x = (x - x.mean(dim=1, keepdim=True)) / (x.std(dim=1, keepdim=True) + 1e-3)
        x = x.view(B, out.shape[-1], -1).permute(0, 2, 1)                    # [B, F, T]
        return x.reshape(B, out.shape[1], out.shape[2], out.shape[3])

    n_samples = int(n_samples)
    out_str = out.clone()

    # 允许最后一组不足 n_samples（ceil 分组）
    n_groups = int(math.ceil(B / n_samples))
    for i in range(n_groups):
        s = i * n_samples
        e = min((i + 1) * n_samples, B)
        if e <= s:
            continue
        chunk = out[s:e]                                                     # [b_i, D, C, T]

        # 展平到 [b_i*T, F]，按列标准化（与原实现一致）
        chunk2 = chunk.reshape(chunk.shape[0], -1, chunk.shape[-1]).permute(0, 2, 1)   # [b_i, T, F]
        chunk2 = chunk2.reshape(chunk2.shape[0] * chunk2.shape[1], -1)                  # [b_i*T, F]
        normed = (chunk2 - chunk2.mean(dim=0)) / (chunk2.std(dim=0) + 1e-3)

        # 还原形状
        normed = normed.view(chunk.shape[0], -1, normed.shape[1]).permute(0, 2, 1)      # [b_i, F, T]
        normed = normed.view(chunk.shape[0], out.shape[1], out.shape[2], -1)            # [b_i, D, C, T]
        out_str[s:e] = normed
